...
# --- Constants and Functions (Safe to run on import) ---
RAW_DATA_PATH = "data/raw.csv"
MODEL_DIR = "models"
MODEL_NAME = "voc_predictor"
N_LAGS = 5
...
def main():
    ...
    print("Starting model training process...")
    # ... (the entire rest of the training logic from your old file goes here) ...
    try:
        df = pd.read_csv(RAW_DATA_PATH)
        if len(df) < N_LAGS + 10:
            print(f"Not enough data to train. Need at least {N_LAGS + 10} rows, but found {len(df)}.")
            return # Use return instead of exit() in a main function
    except FileNotFoundError:
        print(f"Error: Data file not found at {RAW_DATA_PATH}. Run the publisher first.")
        return

    with mlflow.start_run() as run:
        # ... (all the logic inside the with block) ...
        # ...
        print(f"Model saved to: {model_path}")
